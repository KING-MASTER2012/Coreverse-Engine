...
public:
    TempDirGuard()
    {
        ...
        path_ = std::filesystem::temp_directory_path() / ("coreverse_ffi_vfs_test_" + std::to_string(dist(gen)));
...
bool ExpectLastError(std::uint8_t expected_code, const char* what)
{
    ...
    if (code != expected_code) {
        std::fprintf(
            stderr,
            "%s: expected error code %u, got %u\n",
            what,
            static_cast<unsigned>(expected_code),
...
int main()
{
    ...
    if (!ffi_vfs_read_bytes(kRootConfig, "ffi_vfs_test.bin", &read_data, &read_len)) {
        ...
        const bool matches = read_len == payload_len && std::memcmp(read_data, payload_bytes, payload_len) == 0;
        if (!matches) {
