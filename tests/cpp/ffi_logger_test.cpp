...
namespace
{
    ...
    public:
        TempDirGuard()
        {
            ...
            path_ = std::filesystem::temp_directory_path() /
                    ("coreverse_ffi_logger_test_" + std::to_string(dist(gen)));
    ...
    {
        const bool ok =
            ffi_producer_register("CV-FFI-LOGGER-TEST", "FFI Logger Test", "Coreverse");
        if (!ok)
        {
        ...
        const bool duplicate_accepted =
            ffi_producer_register("CV-FFI-LOGGER-TEST", "Duplicate", "Coreverse");
    ...
    bool TestConsoleSink()
    {
        ...
        {
            DiagnosticBuilder* builder = ffi_diagnostic_builder_create(
                "CV-FFI-LOGGER-TEST", "TEST", 1, c.severity, c.message);
            if (builder == nullptr)
            {
    ...
    bool TestFileSink()
    {
        ...
        DiagnosticBuilder* builder = ffi_diagnostic_builder_create(
            "CV-FFI-LOGGER-TEST", "TEST", 2, kSeverityError, kMessage);
        if (builder == nullptr)
        {
            std::fprintf(stderr, "ffi_diagnostic_builder_create() returned null\n");
            ffi_logger_shutdown(logger);
